...
void WifiConfigurationAp::StartWebServer()
{
    ...
    // Register the logo image
    httpd_uri_t logo_png = {
        .uri = "/witblox.png",
        .method = HTTP_GET,
        .handler = [](httpd_req_t *req) -> esp_err_t {
            extern const uint8_t witblox_png_start[] asm("_binary_witblox_png_start");
            extern const uint8_t witblox_png_end[] asm("_binary_witblox_png_end");
            const size_t witblox_png_size = (witblox_png_end - witblox_png_start);
            httpd_resp_set_type(req, "image/png");
            httpd_resp_set_hdr(req, "Connection", "close");
            httpd_resp_send(req, (const char *)witblox_png_start, witblox_png_size);
            return ESP_OK;
        },
        .user_ctx = NULL
    };
    ESP_ERROR_CHECK(httpd_register_uri_handler(server_, &logo_png));
